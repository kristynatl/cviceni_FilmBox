// PŘEDCHYSTANÉ POLE OBJEKTŮ S ÚDAJI O FILMECH
...
filmy.push({
  ...
  nazev: 'Králíček Jojo',
  plakat: {
    url: 'https://www.terryhoponozky.cz/data/products/avatars/41684/watermarked/Kralicek_Jojo_plakat_web.jpg?1594196514',
    sirka: 420,
    vyska: 592,
  },
  ochutnavka: 'Komedie odehrávající se v Německu za druhé světové války',
  popis:
...
const comment = document.querySelector('#note-form');
...
comment.addEventListener('submit', (event) => {
  ...
  if (checkbox.checked && textField.value.length !== 0) {
    return (comment.innerHTML = `<p class="card-text">${textField.value}</p>`);
  }
});
...
document.querySelector('#detail-filmu img').src = currentFilm.plakat.url;
document.querySelector('#detail-filmu .card-title').textContent =
  currentFilm.nazev;
document.querySelector('.card-text').innerHTML = currentFilm.popis;

// Spočítejte kolik dní uběhlo od premiéry nebo za kolik dní premiéra bude. Pomůže vám metoda .diff(). Dnešní datum pro další výpočty v dayjs vytvoříte voláním dayjs(). Datum premiéry pak voláním dayjs('2022-12-24'). Pro vzdálenost mezi datumy pužijte metodu .diff() například takto: dayjs("2022-12-24").diff(dayjs(), 'days'). Do elementu s id premiera připište, před kolika dny nebo za kolik dní bude nebo jestli je dnes.
// EXTRA BONUS - Zařiďte, aby tvar slova den byl ve správném tvaru, aby se třeba nestalo „což bylo před 1 dní“.

let timeElapsed = dayjs(currentFilm.premiera).diff(dayjs(), 'days');
let premiereTense;
let dayWordForm;
if (timeElapsed < 0) {
  timeElapsed = timeElapsed * -1;
  premiereTense = 'bylo před';
  dayWordForm = 'dny';
...
document.querySelector('#premiera').innerHTML = `Premiéra <strong>${dayjs(
  currentFilm.premiera,
).format(
  'D. M. YYYY',
)}</strong>, což ${premiereTense} ${timeElapsed} ${dayWordForm}`;
